...
const authoredQuickReads = Object.entries(editorial).filter(([, entry]) => entry.quickRead);
assert.ok(authoredQuickReads.length > 0, 'The canonical editorial registry must retain authored Quick Read overrides.');
...
assert.match(fieldCard, /<FieldCardQuickRead thing=\{displayThing\} \{country\}/, 'Quick Read must receive country context from the universal Field Card');
...
assert.match(engine, /getEditorialQuickRead/, 'Manual editorial Quick Read overrides must be read from the canonical editorial registry');
...
console.log(`Field Card Quick Read tests passed: ${authoredQuickReads.length} canonical Quick Read override(s) found.`);
