...
assert.deepEqual({ type: thingSnapshot.type, id: thingSnapshot.id, slug: thingSnapshot.slug, name: thingSnapshot.name, country: thingSnapshot.country, city: thingSnapshot.city, category: thingSnapshot.category, shortDescription: thingSnapshot.shortDescription, googleMapsUrl: thingSnapshot.googleMapsUrl, fieldCardPath: thingSnapshot.fieldCardPath }, { type: 'thing', id: 'thing-1', slug: 'walk', name: 'Walk', country: 'laos', city: 'same-city', category: 'things-to-do', shortDescription: 'Walk description', googleMapsUrl: 'https://maps.example/walk', fieldCardPath: '/laos/same-city/things-to-do/walk' });
assert.deepEqual({ src: thingSnapshot.cardImage?.src, alt: thingSnapshot.cardImage?.alt }, { src: '/walk.jpg', alt: 'Walk image' });
